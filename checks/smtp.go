...
func (c Smtp) Run(teamID uint, boxIp string, res chan Result, service Service) {
	// Create a dialer
	dialer := net.Dialer{
	...
	if c.Encrypted {
		conn, err = tls.DialWithDialer(&dialer, "tcp", fmt.Sprintf("%s:%d", boxIp, service.Port), &tlsConfig)
	} else {
