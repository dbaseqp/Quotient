...
func graphTotalScores(data []TeamData, path string, config GraphConfig) error {
	...
	bars, err := plotter.NewBarChart(plotter.Values(scores), vg.Points(50))
	if err != nil {
		return err
	}
	...
	plot.NominalY(placements...)

	plot.Add(bars)
	var xys []plotter.XY
	for i := range scores {
		xys = append(xys, plotter.XY{X: scores[i] + 1, Y: float64(i)})
	}
	...
	for i := range labels.TextStyle {
		labels.TextStyle[i].Color = config.Color
	}
	plot.Add(labels)
