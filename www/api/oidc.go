...
func OIDCCallbackHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Use the OAuth2 token expiry if available, otherwise use role-based expiration
...
func matchesGroup(userGroups []string, configGroup string) bool {
	// Support wildcards
	if strings.Contains(configGroup, "*") {
		pattern := strings.ReplaceAll(configGroup, "*", "")
		for _, userGroup := range userGroups {
			if strings.Contains(userGroup, pattern) {
				return true
			}
		}
		return false
...
func storeOIDCUserInfo(username string, groups []string, roles []string, expiresAt time.Time) {
	oidcUsersMu.Lock()
