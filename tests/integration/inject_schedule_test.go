...
func TestImportedInjectScheduleAnchorsOnceToActualStart(t *testing.T) {
	...
	require.NoError(t, pg.DB.SetCompetitionStarted(true))
	afterStart := time.Now()
	actualStart, err := pg.DB.GetCompetitionStart()
	require.NoError(t, err)
	require.NotNil(t, actualStart)
	assert.False(t, actualStart.Before(beforeStart))
	assert.False(t, actualStart.After(afterStart))

	shifted, err := pg.DB.GetInjectByID(inject.ID)
	require.NoError(t, err)
	...
	require.NoError(t, pg.DB.SetCompetitionStarted(false))
	time.Sleep(10 * time.Millisecond)
	require.NoError(t, pg.DB.SetCompetitionStarted(true))
	restartedAt, err := pg.DB.GetCompetitionStart()
	require.NoError(t, err)
	require.NotNil(t, restartedAt)
	assert.Equal(t, *actualStart, *restartedAt)

	afterRestart, err := pg.DB.GetInjectByID(inject.ID)
	require.NoError(t, err)
